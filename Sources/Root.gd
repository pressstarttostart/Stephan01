...
func _ready():
	...
	main_scene = get_node("Main")
	get_node("Main/Button").set_default_cursor_shape(Control.CURSOR_POINTING_HAND);
	if scenes_node.get_child_count() > 0:
		current_scene = weakref(scenes_node.get_child(root_node.get_child_count() -1))

func read_all_scenes():
...
func _on_Choice_on_selected( path ):
	goto_scene(path)


func _on_Button_pressed():
	var scene = load("res://Levels/Level001/Level001.xml")
	set_new_scene(scene)
